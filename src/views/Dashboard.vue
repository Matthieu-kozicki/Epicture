...
import WeatherCity, { WeatherCityWidget } from '../widgets/Weather/WeatherCity.vue'

export default {
  components: { ImgurSearch, draggable: VueDraggableNext, ImgurProfile, SpotifyArtist, SpotifyProfile, WeatherCity },
  async mounted() {
    this.getWidgets();
    this.getServices();
  },
  methods:
  {
    ...
    initPanels() {
      ...
      if (this.userData.weatherService) {
        this.items[3].items = [
          {label: 'Weather Widget', icon: 'pi pi-fw pi-cloud-upload', command: (event) => { weatherRegister() },},
